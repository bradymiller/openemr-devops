...
if [ "${REDIS_SERVER}" != "" ] &&
   [ ! -f /etc/php-redis-configured ]; then
    ...
    sed -i "s@session.save_handler = files@session.save_handler = redis@" /etc/php${PHP_PKG_VER}/php.ini
    sed -i 's@;session.save_path = "/tmp"@session.save_path = "'"${REDIS_PATH}"'"@"' /etc/php${PHP_PKG_VER}/php.ini
...
if [ "${XDEBUG_IDE_KEY}" != "" ] ||
   ...
   if [ ! -f /etc/php-opcache-jit-configured ]; then
      echo "opcache.enable=0" >> /etc/php${PHP_PKG_VER}/php.ini
   ...
   if [ ! -f /etc/php-opcache-jit-configured ]; then
      echo "opcache.jit=tracing" >> /etc/php${PHP_PKG_VER}/php.ini
      echo "opcache.jit_buffer_size=100M" >> /etc/php${PHP_PKG_VER}/php.ini
      touch /etc/php-opcache-jit-configured
   fi
fi
